...
class LoanService {
  ...
  Future<DocumentFile> uploadDocument(
    ...
    // If it's related to a loan, update the loan's documents list
    if (relatedType == 'loans' && relatedId != null) {
      await _firestore.collection('loans').doc(relatedId).update({
        'documents': FieldValue.arrayUnion([docRef.id])
      });
    }

    final doc = await docRef.get();
    return DocumentFile.fromJson({...doc.data()!, 'id': doc.id});
  }
